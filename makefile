...
$(BINDIR)/openfiles.test: $(OBJDIR)/openfiles.test.o $(OBJDIR)/openfiles.o $(OBJDIR)/netpipefs_file.o $(OBJDIR)/cbuf.o $(OBJDIR)/icl_hash.o $(OBJDIR)/scfiles.o $(OBJDIR)/netpipefs_socket.o $(OBJDIR)/sock.o $(OBJDIR)/utils.o $(OBJDIR)/options.o
	$(CC) $(CFLAGS) $(INCLUDES) -o $@ $^ $(LDFLAGS) $(LIBS)
...
mount_prod: all
	$(BINDIR)/netpipefs -p $(PROD_PORT) --hostip=$(CONS_HOST) --hostport=$(CONS_PORT) --pipecapacity=2048 --timeout=6000 -delayconnect $(PROD_MOUNTPOINT)

mount_cons: all
	$(BINDIR)/netpipefs --port=$(CONS_PORT) --hostip=$(PROD_HOST) --hostport=$(PROD_PORT) --timeout=10000 --pipecapacity=4096 $(CONS_MOUNTPOINT)

debug_prod: all
	$(BINDIR)/netpipefs -p $(PROD_PORT) --hostip=$(CONS_HOST) --hostport=$(CONS_PORT) --pipecapacity=2048 --timeout=6000 --debug -delayconnect $(PROD_MOUNTPOINT)

debug_cons: all
	$(BINDIR)/netpipefs --port=$(CONS_PORT) --hostip=$(PROD_HOST) --hostport=$(PROD_PORT) --timeout=10000 --pipecapacity=4096 -d $(CONS_MOUNTPOINT)
