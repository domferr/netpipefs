...
mount_prod: all
	$(BINDIR)/netpipefs -p $(PROD_PORT) --hostip=$(CONS_HOST) --hostport=$(CONS_PORT) --pipecapacity=65536 --timeout=6000 -delayconnect $(PROD_MOUNTPOINT)

mount_cons: all
	$(BINDIR)/netpipefs --port=$(CONS_PORT) --hostip=$(PROD_HOST) --hostport=$(PROD_PORT) --timeout=10000 --pipecapacity=0 $(CONS_MOUNTPOINT)

debug_prod: all
	$(BINDIR)/netpipefs -p $(PROD_PORT) --hostip=$(CONS_HOST) --hostport=$(CONS_PORT) --pipecapacity=65536 --timeout=6000 --debug -delayconnect $(PROD_MOUNTPOINT)

debug_cons: all
	$(BINDIR)/netpipefs --port=$(CONS_PORT) --hostip=$(PROD_HOST) --hostport=$(PROD_PORT) --timeout=10000 --pipecapacity=0 -d $(CONS_MOUNTPOINT)
